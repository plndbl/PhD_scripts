...
%enter pulse duration from fitting IRF trace (sigma)
pulsewidth =  0.0628

path = ('C:\Users\hl18321\OneDrive - University of Bristol\MAIN_onedrive\PhD\YEAR3\covalent_designs\Paulina\July\')
name = ('design4_F+H_500LP_600SP_POL_445nm_3')

%
%%%PATH FOR SEQUENTIALLY ACQUIRED DATA %%%%%%%%%%%%%%%%
%for April data need to uncomment lines 10-36
...
% for April data comment 49-50

% check the cutoff point - to do it, t = data(2411:4564,1); to t =
% data(:,1); and same for dat; then uncomment line 55

t = data(2411:4564,1);
dat = data(2411:4564,2);
...
F2 = func(B1,B2,sigma,t0,k1,k2,x);

subplot(2,1,1)
hold on
plot(x,F2,'r','DisplayName','No Binding Site');axis([-1 12 0 1.1])
subplot(2,1,2)
hold on
plot(x,F2,'r','DisplayName','No Binding Site');axis([-1 12 1e-5 1.1])
...
%     startguess(8) = 1./guessinput(3);
    ...
    func = @(B1,B2,B3,sigma,t0,k1,x) ((B1.*exp(0.5.*(kq1+k1).*((kq1+k1).*sigma.^2 + 2.*t0 - 2.*x)).*(1 - sqrt(1./sigma.^2).*sigma.*erf(((kq1+k1).*sigma.^2 + t0 - x)./(sqrt(2).*sigma))))./(2.*sqrt(1./sigma.^2).*sigma) + (B2.*exp(0.5.*(kq2+k1).*((kq2+k1).*sigma.^2 + 2.*t0 - 2.*x)).*(1 - sqrt(1./sigma.^2).*sigma.*erf(((kq2+k1).*sigma.^2 + t0 - x)./(sqrt(2).*sigma))))./(2.*sqrt(1./sigma.^2).*sigma) + (B3.*exp(0.5.*(kq3+k1).*((kq3+k1).*sigma.^2 + 2.*t0 - 2.*x)).*(1 - sqrt(1./sigma.^2).*sigma.*erf(((kq3+k1).*sigma.^2 + t0 - x)./(sqrt(2).*sigma))))./(2.*sqrt(1./sigma.^2).*sigma));
...
F3 = func(B1,B2,B3,sigma,t0,k1,x);
subplot(2,1,1)
hold on
plot(x,F3,'b','DisplayName','Design 4');axis([-1 15 0 1.1])
subplot(2,1,2)
hold on
plot(x,F3,'b');axis([-1 15 1e-5 1.1])
    end
end
% 
%change to F3 for triple
%T = table(x,y,F2);
%writetable(T,'C:\Users\hl18321\OneDrive - University of Bristol\MAIN_onedrive\PhD\YEAR3\Paulina\July\design5_FlavinONLY_redlid');
